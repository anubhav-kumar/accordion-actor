    ...
    return (
        <div className='userheading-container'>
            <div className='userheading-image-name'>
                ...
                {isEditMode ? <input style={{margin: '30px 0px'}} onClick={ev => ev.stopPropagation()} onChange={onNameUpdation} type={'text'} defaultValue={`${first} ${last}`} />: <div className='userheading-name'>{`${first} ${last}`}</div>}
            </div>
